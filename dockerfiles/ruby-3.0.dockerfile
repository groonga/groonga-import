...
RUN \
  ...
  git -C mariadb-connector-c fetch origin pull/188/head:rows-events-v2 && \
  git -C mariadb-connector-c checkout rows-events-v2 && \
  git -C mariadb-connector-c rebase 3.2 && \
  git -C mariadb-connector-c checkout - && \
  git -C mariadb-connector-c merge rows-events-v2 && \
